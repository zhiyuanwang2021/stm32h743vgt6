...
static bool CS5552_ConfigChannel(uint8_t channel) {
    ...
    if (channel == 0) {
        ...
        conv_conf = CONV_CONF_CHN_CH0 | CONV_CONF_DR_3200HZ | CONV_CONF_GA_128X; /* 0x000000C1 */
    } else if (channel == 1) {
        reg_os    = REG_OS_CH1;
        reg_gain  = REG_GAIN_CH1;
        reg_conv  = REG_CONV_CONF1;
        conv_conf = CONV_CONF_CHN_CH1 | CONV_CONF_DR_3200HZ | CONV_CONF_GA_128X; /* 0x000010C1 */
    } else {
        return false;
    }
